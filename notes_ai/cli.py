...
from notes_ai.core.cli_app import CLIApp
from notes_ai.core.models.config import Config
from notes_ai.core.tool_impls import LogSearchTool
...
def main():
    parser = argparse.ArgumentParser(description="CLI for Notes AI")
    parser.add_argument(
        '-c',
        '--config',
    ...
    if 'config_file_path' not in args or args.config_file_path is None or args.config_file_path == "":
        cfg_path = os.path.join(NOTES_AI_DATA_DIR, 'config.json')
    else:
