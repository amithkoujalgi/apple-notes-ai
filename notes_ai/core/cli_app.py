...
from notes_ai import NOTES_AI_DATA_DIR
from notes_ai.core.fs_log_rag import NotesAIRAG
from notes_ai.core.models.config import Config
...
class CLIApp:
        ...
        try:
            ...
            if len(response.tool_calls) > 0:
                for tool_call in response.tool_calls:
                    ...
                    exec(
                        f'from notes_ai.core.tool_impls import *;from notes_ai.core.models.config import *;{eval_statement}')
            else:
    ...
    def _ask_llm_raw(self, query: str, stream: bool = True):
        lg = NotesAIRAG(config=self._config)
        lg.scan()
        resp = lg.ask(question=query, stream=stream)

    def scan_and_rebuild_cache(self):
        NotesAIRAG(config=self._config).scan(clean_and_rebuild=True)
    ...
    def start(self):
        ...
        def clear_last():
            # history_file = os.path.join(NOTES_AI_DATA_DIR, 'history.txt')
        ...
        history = FileHistory(filename=os.path.join(NOTES_AI_DATA_DIR, 'history.txt'))
