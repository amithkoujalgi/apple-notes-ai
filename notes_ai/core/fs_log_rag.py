...
from notes_ai import NOTES_AI_DATA_DIR, HUGGING_FACE_EMBEDDINGS_DEVICE_TYPE
from notes_ai.core.models.config import Config, DataSource, Params
...
class NotesAIRAG:
    def __init__(self, config: Config):
        ...
        self._vector_store_directory = os.path.join(NOTES_AI_DATA_DIR, 'cache')
    ...
    def scan(self, clean_and_rebuild: bool = False):
        if clean_and_rebuild or not os.path.exists(self._vector_store_directory):
            ...
            """
            osascript_file = os.path.join(NOTES_AI_DATA_DIR, 'notes.scpt')
            with open(osascript_file, 'w') as f:
                f.write(osascript_text)

            result = subprocess.run(
                ['osascript', osascript_file],
                capture_output=True,  # Capture stdout and stderr
                text=True  # Decode output to string (optional, default is False)
            )

            # Print or process stdout and stderr
    ...
    def _get_response(self, query, chain, stream: bool = False) -> Tuple[str, List[Document]]:
        if stream:
            ...
            response = res.strip()
            self._markdown_print(response)
            return res, src_docs

    def _normal_print(self, text: str):
        print(text, flush=True)
        print('\n')

    def _markdown_print(self, text: str):
        import mdv
        mdv.term_columns = 60
        formatted = mdv.main(text, c_theme=...)
